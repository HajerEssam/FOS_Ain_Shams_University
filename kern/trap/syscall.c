...
{
	uint32 ret = curenv->env_limit;
	return ret;
}
uint32 sys_get_perm(uint32 va)
{
	int ret = pt_get_page_permissions(curenv->env_page_directory, va);
	return ret;
}
...
void* sys_sbrk(int increment)
{
		if(increment==0)
		{
				return (void*)curenv->env_brk;
		}
		int value = increment + curenv->env_brk;
		int value1 = ROUNDUP(value ,PAGE_SIZE);
		if(increment > 0)
		{
		 uint32 Rbrk = curenv->env_brk;
		 if (value1 > curenv->env_limit)
		 {
		 	return (void*) -1;
		 }
		 else
		 {
			 curenv->env_brk = value1;
		 	 allocate_user_mem(curenv,Rbrk, ROUNDUP(increment , PAGE_SIZE));
		 	 return (void*)Rbrk;
		 }
		}
		else if (increment < 0)
		{
			if (value < USER_HEAP_START)
			{
				return (void*) -1;
			}
				cprintf("1");
				uint32 oldb = ROUNDDOWN(curenv->env_brk , PAGE_SIZE);
				uint32 newb = ROUNDUP((increment + curenv->env_brk), PAGE_SIZE);
				if(oldb == newb && curenv->env_brk != oldb)
				{
					cprintf("2");
					free_user_mem(curenv, oldb, PAGE_SIZE);
				}
				else
				{
					cprintf("3");
					...
					{
						cprintf("4");
						free_user_mem(curenv, value1,(NumberOfPages*PAGE_SIZE));
					}
				}
				cprintf("5");
				curenv->env_brk += increment;
				return (void*)curenv->env_brk;
		}
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_sbrk:
		...
		case SYS_free_user_mem:
			if(a1>=USER_LIMIT||(uint32*)a1==NULL||(a1+a2)>=USER_LIMIT)
			{
