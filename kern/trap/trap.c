...
void fault_handler(struct Trapframe *tf)
{
	...
	else
	{
		if (userTrap)
		{
			...
			//If the user tried to access unmarked page
			if(fault_va < USER_HEAP_MAX && fault_va > USER_HEAP_START && (page_perm & PERM_PRESENT) != PERM_PRESENT && (page_perm & PERM_AVAILABLE) != PERM_AVAILABLE)
			{
				sched_kill_env(faulted_env->env_id);
			}
			if((page_perm & PERM_USER) != PERM_USER && (page_perm & PERM_PRESENT)==PERM_PRESENT)
			{
				sched_kill_env(faulted_env->env_id);
			}
			if((page_perm & ~PERM_WRITEABLE) == ~PERM_WRITEABLE && (page_perm & PERM_PRESENT) == PERM_PRESENT)
			{
				sched_kill_env(faulted_env->env_id);
			}
			/*============================================================================================*/
