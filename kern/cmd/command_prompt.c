...
{
		LIST_INIT(&foundCommands);
		char *comname = arguments[0];
		number_of_arguments = number_of_arguments - 1;
		bool found = 0;
		for(int i = 0 ; i < NUM_OF_COMMANDS; i++)
		{
			int difference = strcmp(comname,commands[i].name);
			//check if the commands name are equal
			if(difference == 0)
			{
				found = 1;
				//case : command found and valid
				if(commands[i].num_of_args == number_of_arguments || commands[i].num_of_args == -1)
				{
				...
				//case : the command is found and the num of args not valid
			...
			{
				//the characters subsequence matched case....
				int matched = 0;
				for(int j = 0 ; j < strlen(comname); j++)
				{
					for(int k = j ; k < strlen(commands[i].name);k++)
					{
						if(comname[j] == (char)commands[i].name[k])
						{
							matched++;
							break;
						}
