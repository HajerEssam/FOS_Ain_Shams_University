...
//================
//initialize values needed for env in sched
//load average
...
void env_set_nice(struct Env* e, int nice_value)
{
	...
	fixed_point_t rec=fix_unscale(e->recent,4);

	if(e->env_status!=ENV_NEW)
	{
		e->priority=(num_of_ready_queues-1)-(fix_trunc(fix_unscale(e->recent,4)))-(e->nice_value*2);
...
{
	fixed_point_t x= fix_scale(e->recent,100);
	return fix_round(x);

}
int get_load_average()
{
	fixed_point_t x= fix_scale(load,100);
		return fix_round(x);
