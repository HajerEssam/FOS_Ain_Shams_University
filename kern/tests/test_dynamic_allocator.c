...
void test_realloc_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	if (is_correct)
	{
	  ...
	  //add test
	  cprintf("[5]Add Test calling test_realloc \n\n") ;
	  is_correct = 1;
	  {
	    if(new_size<0){
	      is_correct = 0;
	      cprintf("test_realloc new_size negative .\n");
	    }
	  }
	  if (is_correct)
	  {
	    cprintf("[5.1]correct handel test_realloc new_size is not negative  .\n");
	    eval += 30;
	  }
	    ...
	    is_correct = 1;
	    {
	          if(new_size>sizeof(uint32)){
	          is_correct = 0;
	                cprintf("test_realloc new_size out of range sizeof(uint32).\n");
	      }
	    }
	    if (is_correct)
	    {
	      cprintf("[5.2]correct handel test_realloc new_size in range sizeof(uint32).\n");
	        eval += 30;
	    }
	    is_correct = 1;
	    {
	          if(startVAs[blockIndex]==NULL&&new_size==0&&va!=NULL){
	            is_correct = 0;
	            cprintf("test_realloc return va is must equal null .\n");
	        }
	    }

	  if (is_correct)
	  {
	    cprintf("[5.3]test_realloc return va is equal null when address and new_size equals 0 .\n");
	    eval += 30;
	  }
