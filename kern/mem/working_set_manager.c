...
/*inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address)
{
	...
	if(e->page_last_WS_element == NULL)
	{
		...
		struct WorkingSetElement* WS_element_ptr = &WS_element;
		return WS_element_ptr;
	}
	else
	{
		panic("creating working set element faild : the working set is full!!");
		return NULL;
	}
...
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address )
{
	//cprintf("in env_page_ws_list_create_element \n");
	struct WorkingSetElement* new_ele = (struct WorkingSetElement*)kmalloc(sizeof(struct WorkingSetElement));
	if(new_ele != NULL)
	{
		new_ele->virtual_address= virtual_address;
		//cprintf("env_page_ws_list_create_element end \n");
		return new_ele;
	}
	else panic("no elements");
