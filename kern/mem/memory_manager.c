...
int allocate_frame(struct FrameInfo **ptr_frame_info)
{
	*ptr_frame_info = LIST_FIRST(&free_frame_list);
	...
	if (*ptr_frame_info == NULL)
	{
		...
		if(LIST_SIZE(&env_exit_queue) > 0)
		{
			struct Env* removed_ele = LIST_FIRST(&env_exit_queue);
			env_free(removed_ele);
			dequeue(&env_exit_queue);
		}
		else
		{
			if(isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX))
			{
				...
				{
					LIST_FOREACH(ele , &env_ready_queues[i])
					{
						perm = pt_get_page_permissions(ele->env_page_directory , ele->SecondList.lh_last->virtual_address);
						if(perm && PERM_MODIFIED == PERM_MODIFIED)
						{
							frameinfo = get_frame_info(ele->env_page_directory,ele->SecondList.lh_last->virtual_address , &ptr_page_table);
							pf_update_env_page(ele , ele->SecondList.lh_last->virtual_address , frameinfo);
						}
						env_page_ws_invalidate(ele , ele->SecondList.lh_last->virtual_address);
					}
				}
				perm = pt_get_page_permissions(curenv->env_page_directory , curenv->SecondList.lh_last->virtual_address);
				if(perm && PERM_MODIFIED == PERM_MODIFIED)
				{
					frameinfo = get_frame_info(curenv->env_page_directory,curenv->SecondList.lh_last->virtual_address , &ptr_page_table);
					pf_update_env_page(curenv , curenv->SecondList.lh_last->virtual_address , frameinfo);
				}
			...
			{
				struct Env* ele;
				struct WorkingSetElement *deleted_ws;
				int perm;
				uint32* ptr_page_table = NULL;
				...
				for(int i = 0 ; i < num_of_ready_queues ; i++)
				{
					LIST_FOREACH(ele , &env_ready_queues[i])
					{
						deleted_ws = ele->page_last_WS_element;
						perm = pt_get_page_permissions(ele->env_page_directory,deleted_ws->virtual_address);
						frame = get_frame_info(ele->env_page_directory,deleted_ws->virtual_address,&ptr_page_table);
						if((perm & PERM_MODIFIED) == PERM_MODIFIED)
						{
							pf_update_env_page(ele, deleted_ws->virtual_address,frame);
						}
						env_page_ws_invalidate(ele, deleted_ws->virtual_address);
						unmap_frame(ele->env_page_directory,deleted_ws->virtual_address);
					}
				}
				deleted_ws = curenv->page_last_WS_element;
				perm = pt_get_page_permissions(curenv->env_page_directory,deleted_ws->virtual_address);
				frame = get_frame_info(curenv->env_page_directory,deleted_ws->virtual_address,&ptr_page_table);
				if((perm & PERM_MODIFIED) == PERM_MODIFIED)
				{
