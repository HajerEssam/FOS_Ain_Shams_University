...
void env_free(struct Env *e)
{
	...
	// your code is here, remove the panic and write your code
	{
		if(isPageReplacmentAlgorithmFIFO())
		{
			...
			LIST_FOREACH(ele , &e->page_WS_list)
			{
				env_page_ws_invalidate(e , ele->virtual_address);
				pf_remove_env_page(e , ele->virtual_address);
			}
			free(&e->page_WS_list);
		}
		else if(isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX))
		{
			struct WorkingSetElement* act_ele;
			LIST_FOREACH(act_ele , &e->ActiveList)
			{
				env_page_ws_invalidate(e , act_ele->virtual_address);
				pf_remove_env_page(e , act_ele->virtual_address);
			}
			free(&e->ActiveList);

			struct WorkingSetElement* sec_ele;
			LIST_FOREACH(sec_ele , &e->SecondList)
			{
				env_page_ws_invalidate(e , sec_ele->virtual_address);
				pf_remove_env_page(e , sec_ele->virtual_address);
			}
			free(&e->SecondList);
...
static int program_segment_alloc_map_copy_workingset(struct Env *e, struct ProgramSegment* seg, uint32* allocated_pages, uint32 remaining_ws_pages, uint32* lastTableNumber)
{
	...
	*allocated_pages = 0;
	...
	for (; iVA < end_vaddr && i<remaining_ws_pages; i++, iVA += PAGE_SIZE)
	{
		...
		//cprintf("env_page_ws_list_create_element called hna \n");
		wse->time_stamp = 0;
		LIST_INSERT_TAIL(&(e->page_WS_list), wse);
#else
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
	...
	//	2) call the initialize_dynamic_allocator(..) to complete the initialization
		...
		e->env_limit=daLimit;
		e->env_brk=daStart;
		initialize_dynamic_allocator(e->env_start, 0);
